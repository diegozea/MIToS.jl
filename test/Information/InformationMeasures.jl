@testset "Information Measures" begin
    ...
    @testset "Mutual Information" begin
        ...
        @test mutual_information(probabilities(g,s)) == 0.0
        @test mutual_information(prob_random) ≈ 0.0 # diff for Float64 is 1e-15
        ...
        @test mutual_information(prob_random) ≈ (marginal_entropy(prob_random,1) +
                                                 marginal_entropy(prob_random,2) -
                                                 entropy(prob_random))
        ...
        @testset "MI(X,Y,Z)" begin
            ...
            @test mutual_information(Psss) <= entropy(Pss)
        end
        ...
        @testset "Pairwise Gap Percentage" begin
            ...
            @test gap_union_percentage(count(res"AA--", res"--AA", alphabet=GappedAlphabet(), weight=Float64[.25, .25, .25, .25])) == 100.0
            @test gap_intersection_percentage(count(res"AA--", res"--AA", alphabet=GappedAlphabet(), weight=Float64[.25, .25, .25, .25])) == 0.0
            ...
            @test gap_union_percentage(count(res"AAA-", res"AA--", alphabet=GappedAlphabet(), weights=Float64[.2, .2, .2, .4])) == 60.0
            @test gap_intersection_percentage(count(res"AAA-", res"AA--", alphabet=GappedAlphabet(), weights=Float64[.2, .2, .2, .4])) == 40.0
        end
    end
end
