...
- Fix a bug when writing mmCIF files with `write_file(..., MMCIFFile)` that caused
  an error when the `PDBResidue` doesn't have the element information. For example, 
  this happens when reading PDB files that have truncated atom records, such as
  those generated by Foldseek.
...
- The `Stockholm` file format parser now correctly handles column and residue annotations
  that are split across multiple lines; such as those generated by *HMMER*.

### Changes from v3.2.0 to v3.2.2

- Fixed a bug when parsing mmCIF files that do not include the `pdbx_formal_charge` field; 
  such as those generated by AlphaFold 3.
